...
模型清单以 `shared/models.json` 为准（FastAPI/Workers 统一来源，避免漂移）。

当前主要模型：
- `grok-3` / `grok-3-fast`
- `grok-4` / `grok-4-fast` / `grok-4-heavy` / `grok-4-expert` / `grok-4-fast-expert`
- `grok-4-mini`
- `grok-4.1` / `grok-4.1-thinking`
- `grok-imagine-1.0` / `grok-imagine-1.0-video` / `grok-imagine-0.9`
